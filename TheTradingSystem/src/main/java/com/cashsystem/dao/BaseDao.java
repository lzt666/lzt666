...
public class BaseDao {//连接数据库
        ...
        if (dataSource == null) {
            synchronized(DataSource.class) {
                if (dataSource == null) {
                    ...
                    ((MysqlDataSource)dataSource).setUseUnicode(true);
                    ((MysqlDataSource)dataSource).setCharacterEncoding("utf8");
                    ((MysqlDataSource) dataSource).setUser("root");
                    ((MysqlDataSource) dataSource).setPassword("56789");
                }
            }
        }
        return dataSource;
    ...
    protected void closeResource(ResultSet resultSet, PreparedStatement statement, Connection connection) {//关闭资源的函数
        //结果 -> 命令 -> 连接
        if (resultSet != null) {
            try {
                resultSet.close();
